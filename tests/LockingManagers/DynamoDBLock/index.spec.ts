...
describe('DynamoDBLock', () => {
  ...
  test('extendLock should extend the lock duration', async () => {
    ...
    const extendResult = await lockManager.extendLock(
      path,
      acquireResult!.lockId!,
      extensionDuration,
    );
    ...
    console.log({
      extendedExpiresAt,
      initialExpiresAt,
      diff: extendedExpiresAt - initialExpiresAt,
    });
    expect(extendedExpiresAt).toBeGreaterThan(initialExpiresAt);
    expect(extendedExpiresAt - initialExpiresAt).toBeCloseTo(
      extensionDuration,
      -2,
  ...
  test('acquireLock should succeed immediately after an expired lock is deleted', async () => {
    ...
    // Acquire a lock with a short timeout
    const firstAcquireResult = await lockManager.acquireLock(path, {
      timeout: shortTimeout,
    });
    expect(firstAcquireResult.success).toBe(true);

    const failedTryToAcquireLock = await lockManager.acquireLock(path, {
      timeout: 1000,
    });
